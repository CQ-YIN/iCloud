...
async function displayImageCounts() {
    try {
        ...
        let userDrops = await contract.getUserAmounts(userAddress)
        console.log("userDrops", userDrops)



        const rainCount = await contract.rain();
        const roseCount = await contract.rose();
        const bitcoinCount = await contract.bitcoin();
...
async function displayUserAmounts() {
    try {
        const { _rain, _rose, _bitcoin } = await contractWithSigner.getUserAmounts(await signer.getAddress());
...
async function displayPreviousTotals() {
    try {
        ...
        // 同样地，进行检查
        if (_rain !== undefined && _rose !== undefined && _bitcoin !== undefined) {
            document.getElementById('totalRain').innerText = `We have ${_rain - 1} raindrops!`;
            document.getElementById('totalRose').innerText = `We have ${_rose - 1} roses!`;
            document.getElementById('totalBitcoin').innerText = `We have ${_bitcoin - 1} bitcoins!`;
        } else {
            console.error("Returned data from smart contract is undefined.");
        }
    } catch (error) {
